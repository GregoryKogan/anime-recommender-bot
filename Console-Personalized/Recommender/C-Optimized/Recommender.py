...
def get_recs(anime_ids_list, anime_id_ind_dict, user, user_factors, progress_log=False):
    recommendations = []
    watched_titles = list(user)
    titles_done = 0
    threads = []
...
def get_recs_for_user_by_factors(user, user_factors, progress_log=False):
    anime_ids = get_anime_ids()
    anime_id_ind_dictionary = get_anime_id_ind_dict(anime_ids)
    import time
    begin = time.perf_counter()
    recs = get_recs(anime_ids, anime_id_ind_dictionary, user, user_factors, progress_log=progress_log)
